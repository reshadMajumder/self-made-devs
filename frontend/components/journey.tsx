...
const quests = [
  {
    ...
    skills: ["HTML & CSS", "JavaScript ES6+", "Git & GitHub", "Command Line"],
    xp: "1000 XP",
  ...
  },
  {
    ...
    description: "Conquer backend development and become a full-stack developer.",
    skills: ["Node.js", "Databases", "Authentication", "REST APIs"],
    xp: "5000 XP",
  ...
  },
  {
    ...
    description: "Deploy your masterpiece and join the ranks of production developers.",
    skills: ["Deployment", "Testing", "CI/CD", "Portfolio Building"],
    xp: "10000 XP",
      ...
      (entries) => {
        entries.forEach((entry) => {
          if (entry.isIntersecting) {
            // Unlock quests one by one with delay
      ...
      <div className="container mx-auto relative z-10">
        <div className="text-center mb-20 space-y-4">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-gradient-to-r from-blue-500/20 to-purple-500/20 border border-white/10 backdrop-blur-sm mb-4">
            ...
            <span className="text-sm font-semibold text-white">Quest System</span>
          </div>
          ...
          <p className="text-xl text-white/70 max-w-2xl mx-auto text-balance">
            Complete each quest to unlock the next level. Your transformation awaits.
          </p>
        </div>
            ...
            return (
              <div
                ...
                className={`relative transition-all duration-700 ${
                  isUnlocked ? "opacity-100 translate-x-0" : "opacity-30 translate-x-10"
                }`}
              >
                  ...
                  {isLocked && (
                    <div className="absolute inset-0 flex items-center justify-center bg-black/60 backdrop-blur-sm z-10">
                      <div className="text-center space-y-2">
                        ...
                        <p className="text-white/60 font-semibold">Complete Previous Quest to Unlock</p>
                      </div>
                    </div>
                  )}
                  ...
                  <div className="relative p-8 md:p-10">
                    <div className="flex flex-col md:flex-row gap-8">
                      ...
                      <div className="flex-shrink-0">
                        <div
                          className={`relative w-24 h-24 rounded-2xl bg-gradient-to-br ${quest.color} flex items-center justify-center shadow-2xl ${
                        ...
                        >
                          ...
                          <div className="absolute -top-2 -right-2 w-8 h-8 rounded-full bg-black border-2 border-white/20 flex items-center justify-center">
                            <span className="text-xs font-bold text-white">{quest.level}</span>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="flex-1 space-y-4">
                        <div>
                          <div className="flex items-center gap-3 mb-2">
                            <span className="text-sm font-semibold text-white/60">{quest.month}</span>
                            <span className="text-sm font-semibold text-yellow-400">{quest.xp}</span>
                            {isUnlocked && <Star className="w-4 h-4 text-yellow-400 animate-pulse" />}
                          </div>
                          <h3 className="text-3xl font-bold text-white mb-1">{quest.title}</h3>
                          <p
                        ...
                        <p className="text-white/70 text-lg">{quest.description}</p>
                          ...
                          {quest.skills.map((skill, skillIndex) => (
                            <div
                              ...
                              className={`flex items-center gap-2 px-4 py-2 rounded-lg bg-white/5 border border-white/10 transition-all duration-300 ${
                                isUnlocked ? "hover:bg-white/10 hover:border-white/20" : ""
                              }`}
                            >
                              <CheckCircle2
                                className={`w-4 h-4 flex-shrink-0 ${isUnlocked ? "text-green-400" : "text-white/20"}`}
                              />
                              <span className={`text-sm ${isUnlocked ? "text-white" : "text-white/40"}`}>{skill}</span>
                            </div>
                          ))}
                        </div>
                      </div>
        ...
        <div className="text-center mt-16 space-y-4">
          <div className="inline-flex items-center gap-2 px-6 py-3 rounded-full bg-gradient-to-r from-yellow-500/20 to-amber-500/20 border border-yellow-500/30 backdrop-blur-sm">
            ...
            <span className="font-semibold text-yellow-400">Complete All Quests to Become a Self Made Dev</span>
          </div>
        </div>
      </div>
    </section>
